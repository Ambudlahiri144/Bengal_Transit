...
export default function AdminHub() {
  ...
  useEffect(() => {
    ...
    if (parsedUser.role !== 'ADMIN') {
      router.push('/'); // Kick unauthorized users to home
      return;
    }
  ...
  return (
    <main className="min-h-[100dvh] bg-gray-50 flex flex-col items-center justify-center p-4 sm:p-6 font-sans relative">
      ...
      {/* 👇 Scaled down logo for mobile, larger on desktop 👇 */}
      ...
      {/* Header - Added extra top margin on mobile to clear the logo */}
      ...
      {/* Cards Container */}
      <div className="flex flex-col md:flex-row gap-6 md:gap-10 max-w-5xl w-full px-2 sm:px-4">
        
        {/* Card 1: Data Dashboard */}
        <Link href="/admin/dashboard" className="group flex-1 bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden relative transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 md:hover:-translate-y-2 cursor-pointer h-72 md:h-80 flex flex-col">
          <div className="p-6 md:p-8 pb-0 flex-1">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">Data Dashboard</h2>
            <p className="text-xs md:text-sm text-gray-500 leading-relaxed">
              View comprehensive Excel-style reports of all bookings, passengers, and transactions. Export directly to CSV.
            </p>
          </div>
        ...
        {/* Card 2: Travel Agent Mode */}
        <div onClick={() => router.push('/')} className="group flex-1 bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden relative transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 md:hover:-translate-y-2 cursor-pointer h-72 md:h-80 flex flex-col">
          <div className="p-6 md:p-8 pb-0 flex-1">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">Travel Agent Mode</h2>
            <p className="text-xs md:text-sm text-gray-500 leading-relaxed">
              Redirect to the primary booking portal. Tickets booked here will be logged under your Admin credentials.
            </p>
          </div>
          {/* Hover Preview Graphic */}
